...
module.exports = class API {
                ...
                bcrypt.compare(call.request.getPassword(), user.password, (err, result) => {
                    if (result) {
                        ...
                        callback({
                            ...
                            message: "No user found asdadasd",
                        });
                    }
    ...
    changeUserEmail = (call, callback) => {
        const users = this.db.collection("users");
        let resp = new messages.VerifyResponse();
        users.findOne({ _id: ObjectId(call.request.getId()) }).then(user => {
            if (user) {
                users.updateOne({ _id: ObjectId(call.request.getId()) }, { email: call.request.getEmail() }).then(user => {
                    if (user) {
                        resp.setId(user._id.toString());
                        resp.setName(user.name);
                        resp.setEmail(user.email);
                        ...
                        return callback({
                            ...
                            message: "User found but update failed :(",
                        });
                    }
                });
            } else {
